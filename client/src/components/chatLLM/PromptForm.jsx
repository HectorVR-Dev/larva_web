...
const PromptForm = ({ conversations, setConversations, activeConversation, generateResponse, isLoading, setIsLoading }) => {
  ...
  return (
    <form className={`${styles.promptForm}`} onSubmit={handleSubmit}>
      <input placeholder="Message Gemini..." className={`${styles.promptInput}`} value={promptText} onChange={(e) => setPromptText(e.target.value)} required />
      <button type="submit" className={`${styles.sendPromptBtn}`}>
        <ArrowUp size={20} />
      </button>
    </form>
  );
