...
const PromptForm = ({ conversations, setConversations, activeConversation, generateResponse, isLoading, setIsLoading, placeholder = 'Mensaje...' }) => {
  ...
  return (
    <form className="chatbot-prompt-form" onSubmit={handleSubmit}>
      <input placeholder={placeholder} className="chatbot-prompt-input" value={promptText} onChange={(e) => setPromptText(e.target.value)} required />
      <button type="submit" className="chatbot-send-prompt-btn">
        <ArrowUp size={20} />
      </button>
    </form>
  );
